...
export async function GET(req: NextRequest) {
  ...
  // If template requested, validate it exists via HTTP fetch (works across edge/node isolates)
  if (templateId) {
    try {
  ...
  try {
    ...
    if (templateId) {
      ...
      // Build enriched rows by swapping image_link; keep product handle mapping via link column
      rows = rows.map((r) => {
